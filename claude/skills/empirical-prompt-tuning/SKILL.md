...
description: agent 向けテキスト指示（skill / slash command / task プロンプト / CLAUDE.md 節 / コード生成プロンプト）を、対象の目的を変えずに、白紙の実行者・固定シナリオ・hold-out で評価し、最小修正を反復する skill 改善オーケストレータ。プロンプトや skill を新規作成・大幅改訂した直後、またはエージェントの挙動不良の原因を指示側の曖昧さに求めたいときに使う。
---
...
実行順は `inspect → baseline → fresh dispatch → diagnose → one-theme patch → fresh re-run → hold-out → report` に固定する。対象 skill を使って本来のタスクを実行する依頼と、対象 skill 自体を改善する依頼を混同しない。後者では production code、commit、push、PR は対象に含めず、必要なら別の明示依頼として扱う。

シナリオが未保存なら、実運用の中央値 1 本と edge 1〜2 本を先に固定する。既存の失敗例がある場合はそれを優先し、修正後にシナリオを簡単にして合格させない。評価を終えたら、各修正がどの要件文言に効いたか、未解決の曖昧さ、次に回す課題をレポートする。
...
0. **Iteration 0 — description と body の整合チェック**（静的、dispatch 不要）
   ...
   - 結果を「description の主張」「body の対応」「判定（一致 / 不一致）」「正確な alignment diff」の4項目で記録する。差分が無ければ「なし」と明記する
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - Agent tool が usage メタを返さない場合は `tool_uses` / `duration_ms` を「取得不可」と記録し、推測値で埋めない。数値条件に依存する収束判定は未確定として扱い、メタデータを取得できる親セッションで再評価する
...
- body の対応: ...
- 判定: 一致 / 不一致
- alignment diff: 不一致なら description にだけある用途 / body にだけある手順。なければ「なし」
...
### 実行結果（シナリオ別）
| シナリオ | 成功/失敗 | 精度 | steps（または取得不可） | duration（または取得不可） | retries |
|---|---|---|---|---|---|
| A | ○ | 90% | 取得不可 | 取得不可 | 0 |
| B | × | 60% | 9 | 41s | 2 |
※ usage メタが無い場合は `steps` / `duration` に「取得不可」と記録し、数値収束は未確定とする。
...
- `skills/skill-creator/` — 新規 skill の作成ガイド。本 skill はその完成後の検証フェーズで使う
- `skills/grill-implementation/` — 実装プランの stress-test。本 skill はプロンプトの stress-test
- `rules/core-standards.md`（「過去の教訓」節） — 過去の修正指示の蓄積。改訂後、同じ曖昧さが再発していないか照合する
- Agent tool の並列 dispatch — 複数シナリオを同時に走らせるときは単一メッセージ内で複数 Agent 呼び出しを並べる

## 保存済みシナリオ

`scenarios/` 配下に対象 skill ごとの評価シナリオを md で保存している。対象を改訂したら該当ディレクトリを読み、`## subagent 投入プロンプト` セクションをコピペして Agent tool で dispatch → 戻り値を `## 要件チェックリスト` と照合 → 結果を `## 最終実行結果` テーブルに追記する。詳細は `scenarios/README.md`。
