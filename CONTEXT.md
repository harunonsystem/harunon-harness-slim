...
| hookRunner | Claude PreToolUse hook（`packages/core/hooks/*.sh`）を Claude 以外の runtime で実行する共有 module（`packages/core/hook-runner/hook-runner.js`）。interface は `createHookRunner({ runtime }).preToolUse(toolName, toolInput, cwd) -> { decision, reason, finalInput, warnings }` の 1 本で、hookPipeline の実行時読み込み・wire protocol（stdin JSON / exit 2 / hookSpecificOutput / updatedInput 連鎖）・`required` hook の fail-closed・spawn の作法を内側に持つ。runtime ごとの adapter（pi: `pi-extensions/claude-hooks-bridge.ts`、omp: `omp-extensions/omp-denial-reason.js`、opencode: `opencode-plugins/claude-hooks-bridge.js`）は runtime のツール名 / 入力を Claude 名に写し、decision をホストの block / confirm / throw に写すだけ。配布レイアウト（`hook-runner/` の隣に `claude-hooks/` と `policy/`）が 3 runtime で同じ相対関係になる契約。post-edit（write / edit 後の shellcheck / jq / GFM）も同型で、`hook-runner/post-edit.js` が `post-edit-checks.sh` を呼び、adapter は path の取り出しと result への追記だけを持つ。codexはPython（`codex_hook.py`）が同じtableを読む第2実装で、hookProtocol の conformance fixture が adapter 差（確認 UI の有無・updatedInput の書き戻し）を divergence として宣言する |
...
| executor | assignmentを実行する作業者runtimeの識別子。`task-state.schema.json` とkernelは空でない文字列を検証し、roleはimplement / reviewに限定する。起動方法とruntimeの選択はadapter側の責務 |
| transport | workerへのdispatchに使った通信経路の識別子。adapterがtransport・ref・atを渡し、kernelはdispatchの事実を記録する。kernelは通信を実行しない |
...
packages/core/ (SSOT)
    ...
    ├── hooks/        ← フック群（Claude Code 用。pi / omp / opencode はhookRunner、codexはPython adapter経由で実行）
...
ガード:
- pre-pushがClaude targetのライブ ↔ SSOTドリフトを警告。他targetは `/sync-settings --check all` で確認
- bootstrap が上書き発生時に件数と backup 先を警告（還流要否の確認を促す）
- 巻き戻してしまった場合は `~/.claude/backups/bootstrap-*/` から復元して還流する（backup は最新5件保持）
...
| skills 配布 | コピー (bootstrap) | `~/.agents/skills`（shared-agents）を参照 | `~/.agents/skills`（shared-agents）を参照 | `~/.agents/skills`（shared-agents）を参照 | `~/.agents/skills`（shared-agents）を参照 |
