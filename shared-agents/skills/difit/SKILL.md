---
name: difit
description: Use when the user wants review findings shown in difit's UI or wants to visually inspect the diff. Ask the user for a code review through difit after code changes, or review a specific diff (branch, commit, or GitHub PR) and preload the findings as `--comment` threads before launching difit for the user.
---
...
When the user asks to *review* a specific diff (branch diff, commit, or GitHub PR) and show the findings inside difit:

1. Identify the target diff and review its contents. Inspect surrounding code when needed and prepare review findings or explanations.
...
2. Attach the prepared findings as `--comment` threads (format above) and launch difit.
   - For stdin input, use a form such as `diff -u file1.txt file2.txt | <difit-command>`.
3. Share the difit URL and finish the response. If there were no comments to attach, explicitly say so.
