...
- Design System Rules（同ディレクトリの `figma-design-system-rules.md`）を Read し、
  Figma変数→プロジェクトトークンの変換にマッピングテーブルを使用する。
...
1. Storybook devサーバーを起動し、ブラウザツールでコンポーネントをスクリーンショット
...
`pnpm tsc --noEmit` と `pnpm lint` を実行。最終スクリーンショット比較を提示。
